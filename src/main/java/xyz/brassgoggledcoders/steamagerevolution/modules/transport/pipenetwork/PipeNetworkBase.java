 ...
 */
public abstract class PipeNetworkBase implements IMultiblockValidator {
	...
	protected HashSet<IPipe> connectedParts;
	...
	protected PipeNetworkBase(World world) {
		// Multiblock stuff
		WORLD = world;
		connectedParts = new HashSet<IPipe>();
	 ...
	 */
	public abstract void onAttachedPartWithMultiblockData(IPipe part, NBTTagCompound data);
	 ...
	 */
	public void attachBlock(IPipe part) {
		...
		if(this.referenceCoord == null) {
			...
			((IPipe) te).forfeitMultiblockSaveDelegate();
	 ...
	 */
	protected abstract void onBlockAdded(IPipe newPart);
	 ...
	 */
	protected abstract void onBlockRemoved(IPipe oldPart);
	 ...
	 */
	private void onDetachBlock(IPipe part) {
		// Strip out this part
	 ...
	 *            instead of broken.
	 */
	public void detachBlock(IPipe part, boolean chunkUnloading) {
	 ...
	 */
	private void formNetwork(AssemblyState oldState) {
		for(IPipe part : connectedParts) {
	 ...
	 */
	public void assimilate(PipeNetworkBase other) {
	 ...
	 */
	private void _onAssimilated(PipeNetworkBase otherController) {
		if(referenceCoord != null) {
	 ...
	 */
	protected abstract void onAssimilate(PipeNetworkBase assimilated);
	 ...
	 */
	protected abstract void onAssimilated(PipeNetworkBase assimilator);

	/**
	 * Driver for the update loop. If the machine is assembled, runs
	 * the game logic update method.
	 *
	 * @see PipeNetworkBase#updateServer()
	 */
	...
	public void recalculateMinMaxCoords() {
		...
		for(IPipe part : this.connectedParts) {
	 ...
	 */
	public boolean shouldConsume(PipeNetworkBase otherController) {
	...
	private int _shouldConsume(PipeNetworkBase otherController) {
	...
	private void auditParts() {
		HashSet<IPipe> deadParts = new HashSet<IPipe>();
		for(IPipe part : connectedParts) {
	 ...
	 * @return
	 */
	public Set<IPipe> checkForDisconnections() {
		...
		Set<IPipe> deadParts = new HashSet<IPipe>();
		BlockPos position;
		IPipe referencePart = null;
		...
		IPipe part;
		LinkedList<IPipe> partsToCheck = new LinkedList<IPipe>();
		IPipe[] nearbyParts = null;
		int visitedParts = 0;
		...
		while(!partsToCheck.isEmpty()) {
			...
			for(IPipe nearbyPart : nearbyParts) {
		...
		Set<IPipe> removedParts = new HashSet<IPipe>();
		for(IPipe orphanCandidate : connectedParts) {
	 ...
	 */
	public Set<IPipe> detachAllBlocks() {
		if(WORLD == null) {
			return new HashSet<IPipe>();
		}
		...
		Set<IPipe> detachedParts = connectedParts;
		connectedParts = new HashSet<IPipe>();
	 ...
	 *
	 * @see PipeNetworkBase#markReferenceCoordForUpdate()
	 */
