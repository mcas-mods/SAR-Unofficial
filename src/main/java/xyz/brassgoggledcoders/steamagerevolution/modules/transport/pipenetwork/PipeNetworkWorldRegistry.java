...
final class PipeNetworkWorldRegistry {

	private World worldObj;

	private final Set<PipeNetworkBase> controllers; // Active controllers
	private final Set<PipeNetworkBase> dirtyControllers; // Controllers whose parts lists have changed
	private final Set<PipeNetworkBase> deadControllers; // Controllers which are empty
	...
	private Set<IPipe> orphanedParts;

	// A list of parts which have been detached during internal operations
	private final Set<IPipe> detachedParts;
	...
	// Indexed by the hashed chunk coordinate
	// This can be added-to asynchronously via chunk loads!
	private final HashMap<Long, Set<IPipe>> partsAwaitingChunkLoad;
	...
	public PipeNetworkWorldRegistry(final World world) {
		worldObj = world;

		controllers = new HashSet<PipeNetworkBase>();
		deadControllers = new HashSet<PipeNetworkBase>();
		dirtyControllers = new HashSet<PipeNetworkBase>();

		detachedParts = new HashSet<IPipe>();
		orphanedParts = new HashSet<IPipe>();

		partsAwaitingChunkLoad = new HashMap<Long, Set<IPipe>>();
	...
	public void tickStart() {
		if(controllers.size() > 0) {
			for(PipeNetworkBase controller : controllers) {
	...
	public void processMultiblockChanges() {
		...
		List<Set<PipeNetworkBase>> mergePools = null;
		if(orphanedParts.size() > 0) {
			Set<IPipe> orphansToProcess = null;
			...
			if(orphansToProcess != null && orphansToProcess.size() > 0) {
				...
				for(IPipe orphan : orphansToProcess) {
					...
					if(compatibleControllers == null) {
						...
						List<Set<PipeNetworkBase>> candidatePools = new ArrayList<Set<PipeNetworkBase>>();
						for(Set<PipeNetworkBase> candidatePool : mergePools) {
						...
						if(candidatePools.size() <= 0) {
							...
							Set<PipeNetworkBase> masterPool = candidatePools.get(0);
							Set<PipeNetworkBase> consumedPool;
		...
		if(mergePools != null && mergePools.size() > 0) {
			...
			for(Set<PipeNetworkBase> mergePool : mergePools) {
				// Search for the new master machine, which will take over all the blocks contained in the other
				// machines
				PipeNetworkBase newMaster = null;
				for(PipeNetworkBase controller : mergePool) {
		...
		if(dirtyControllers.size() > 0) {
			Set<IPipe> newlyDetachedParts = null;
			for(PipeNetworkBase controller : dirtyControllers) {
		...
		if(deadControllers.size() > 0) {
			for(PipeNetworkBase controller : deadControllers) {
	 ...
	 */
	public void onPartAdded(final IPipe part) {
	 ...
	 */
	public void onPartRemovedFromWorld(final IPipe part) {
	 ...
	 *
	 * @param controller The controller which is dead.
	 */
	public void addDeadController(PipeNetworkBase controller) {
	 ...
	 */
	public void addDirtyController(PipeNetworkBase dirtyController) {
	 ...
	 */
	public Set<PipeNetworkBase> getControllers() {
		return Collections.unmodifiableSet(controllers);
	}

	/* *** INTERNAL HELPERS *** */

	protected IPipe getMultiblockPartFromWorld(final World world, final BlockPos position) {

		TileEntity te = world.getTileEntity(position);

		return te instanceof IPipe ? (IPipe) te : null;
	}

	private void addOrphanedPartThreadsafe(final IPipe part) {
		synchronized(orphanedPartsMutex) {
			orphanedParts.add(part);
		}
	}

	private void addAllOrphanedPartsThreadsafe(final Collection<? extends IPipe> parts) {
		synchronized(orphanedPartsMutex) {
