package xyz.brassgoggledcoders.steamagerevolution;

import net.minecraft.block.Block;
import net.minecraft.item.Item;
import net.minecraft.item.ItemMinecart;
import net.minecraftforge.fml.common.registry.GameRegistry.ObjectHolder;

@ObjectHolder(SteamAgeRevolution.MODID)
public class SARObjectHolder {
    public static final Block alloy_furnace_frame = null;
    public static final Block boiler_frame = null;
    public static final Item canister = null;
    public static final Block casting_bench = null;
    public static final Block charcoal_block = null;
    public static final Item charcoal_powder = null;
    public static final Block crucible_casing = null;
    public static final Item die = null;
    public static final Block distiller_frame = null;
    public static final Block distiller_hotplate = null;
    public static final Block distiller_radiator = null;
    public static final Item drill = null;
    public static final Block fume_collector = null;
    public static final Block furnace_casing = null;
    public static final Item goggles = null;
    public static final Block grinder_frame = null;
    public static final Item gun = null;
    public static final Item hammer = null;
    public static final Block incense = null; // Refers to fluid block not solid
    public static final Block incense_block = null;
    public static final Block incense_burner = null;
    public static final Block incense_spray = null;
    public static final Item lens = null;
    public static final ItemMinecart minecart_carrier = null;
    public static final ItemMinecart minecart_drilling = null;
    public static final Block pneumatic_router = null;
    public static final Block pneumatic_sender = null;
    public static final Block pneumatic_tube = null;
    public static final Item rocket_fist = null;
    public static final Block sawmill_casing = null;
    public static final Block steamhammer_anvil = null;
    public static final Block steamhammer_frame = null;
    public static final Block steelworks_frame = null;
    public static final Block vat_output = null;
    public static final Item wings = null;
    public static final Block portable_boiler = null;
    public static final Block alloy_furnace_fluid_input = null;
    public static final Block alloy_furnace_fluid_output = null;
}
