...
public class VatRecipeCategory extends SARRecipeCategory<SARMachineRecipe> {
	...
	public void setRecipe(IRecipeLayout recipeLayout, SARMachineRecipe recipeWrapper, IIngredients ingredients) {
		...
		// FluidStack stack = recipeWrapper.getFluidInputs()[i];
		// if(stack != null) {
		...
		// null);
		// recipeLayout.getFluidStacks().set(i, recipeWrapper.getFluidInputs()[i]);
		// }
		// }
		// if(recipeWrapper.getItemInputs() != null) {
		// for(int i2 = 0; i2 < recipeWrapper.getItemInputs().length; i2++) {
		// ItemStack stack = recipeWrapper.getItemInputs()[i2];
		// if(!stack.isEmpty()) {
		// recipeLayout.getItemStacks().init(i2 + 3, true, 2 + (i2 * 40) - i2, 105);
		// recipeLayout.getItemStacks().set(i2 + 3, recipeWrapper.getItemInputs()[i2]);
		// }
		// }
		// }
		// recipeLayout.getFluidStacks().init(6, false, 124, 50, 20, 60,
		// ControllerVat.outputCapacity, true, null);
		// recipeLayout.getFluidStacks().set(6,
		// ingredients.getOutputs(FluidStack.class).get(0));
		//
